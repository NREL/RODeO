...
@author: pgagnon
Updated by Josh Eichman

Sometimes needs to run twice to help initailze
"""

support_repo_path = 'C:/Users/jeichman/Documents/Tariff_analysis/support_functions/python'
Analysis_path = 'C:/Users/jeichman/Documents/gamsdir/projdir/RODeO/Projects/Test'

Output_files = Analysis_path + '/Data_files'
Input_files = Output_files + '/CSV_data/Profile_selection'
...
# Import a reference load profile (only used if you want to calaculate the cost to operate on that rate)
load_profile = np.genfromtxt(Input_files + '/ref_profile.csv') 

# Load all tariff ids
load_ids = np.genfromtxt(Input_files + '/list_of_rates.csv', dtype=str, delimiter=",")
"""
...
for t_id in range(id_length):        #range(0,300,1):    #e_max_diff error with 243, 635, 703, 828, 1684, 2772    ?2268, 3803
    ...
    # Check if the json filesize is >50bytes and if so use it, otherwise populate the json file then use it
    try:
...
# missed_values = list([88,92,94,97,100,103,106,109,112,115,304])
# tariff_id_missed = load_ids[missed_values]
