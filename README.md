...
*elec_rate_instance*: Tthe retail rate used. If *run_retail_instance* is set to use wholesale rates then it doesn't matter what rate is selected here.
...
*storage_set_instance*: Toggles a feature that sets the inital volume and end volume levels (particularly useful for long-duration energy storage)
*storage_init_instance*: Setpoint for the inital volume level as a fraction of the entire storage system [0 to 1]
...
*wacc_instance*: Weighted Average Cost of Capital (this value is calaculted based on the next values)
...
*op_period_instance*: Number of intrevals in each operating period (rolling solution window)
...
*CF_opt_instance*: Toggle to enable or disable optimization of the capaicty factor
...
**Low Carbon Fuel Standard (LCFS) parameters**
