...
$if not set elec_rate_instance     $set elec_rate_instance     586fd4265457a306691c9607_hourly
$if not set add_param_instance     $set add_param_instance     additional_parameters_central_hourly
...
$if not set outdir                 $set outdir                 RODeO\Output\Central_vs_distributed
$if not set indir                  $set indir                  RODeO\data_files\Central_vs_distributed\Output\TXT_files
...
$if not set input_lifetime_inst    $set input_lifetime_inst    0
$if not set output_lifetime_inst   $set output_lifetime_inst   0
$if not set interest_rate_inst     $set interest_rate_inst     0
...
$if not set file_name_instance     $set file_name_instance     "Test_central"
...
*set operating period length to full year length (8760 or 8784) to do full-year optimization without rolling window
         ...
         price_cap               'Set price cap'                                                                 /%price_cap_instance%/
;
...
storage_level_accounting_final_eqn(interval)$(rolling_window_min_index <= ord(interval) and ord(interval) <= rolling_window_max_index and input_capacity_MW > 0 and baseload_operation=0 and ord(interval)=operating_period_length )..
