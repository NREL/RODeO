...
$if not set ren_prof_instance      $set ren_prof_instance      renewable_profiles_none_v2_hourly
$if not set load_prof_instance     $set load_prof_instance     Additional_load_none_v2_hourly
...
*  Revert to 1D array
         ...
         renewable_signal(interval,devices)      "Renewable generation signal (MW) (range from 0 to 1)"
         meter_mnth_chg(interval)                "monthly charge for meter ($/meter/month)"
         Renewable_power(interval,devices)       "Scaled renewable signal based on 'Renewable_MW'"
         Fixed_dem(months)                       "Fixed demand charge $/MW-month"
         Timed_dem(timed_dem_period)             "Timed demand charge $/MW-month"
         Load_profile(interval,devices)          "Load profile (MW)"
...
*         ones_mat(interval)                      "define all elements to 10"     /set.interval 1/;
         ...
         renew_cap_cost(devices)                 "upfront capital cost ($/MW)"                           /1 %renew_cap_cost_inst%, 2 %renew_cap_cost_inst%/
         ...
         H2stor_cap_cost(devices)                "upfront capital cost ($/kg)"                           /1 %H2stor_cap_cost_inst%, 2 %H2stor_cap_cost_inst%/
         renew_FOM_cost(devices)                 "upfront FOM cost ($/MW-year)"                          /1 %renew_FOM_cost_inst%, 2 %renew_FOM_cost_inst%/
         ...
         renew_VOM_cost(devices)                 "upfront VOM cost ($/MW-year)"                          /1 %renew_VOM_cost_inst%, 2 %renew_VOM_cost_inst%/
         ...
         renew_lifetime(devices)                 "equipment lifetime (years)"                            /1 %renew_lifetime_inst%, 2 %renew_lifetime_inst%/
         ...
         renew_interest_rate(devices)            "interest rate on debt for renewables"                  /1 %renew_interest_inst%, 2 %renew_interest_inst%/
         ...
         NG_price_adj(devices)                   "Average price of natural gas ($/MMBTU)"                        /1 %NG_price_adj_instance%, 2 %NG_price_adj_instance%/
         Renewable_MW(devices)                   "Installed renewable capacity (MW)"                             /1 %Renewable_MW_instance%, 2 %Renewable_MW_instance%/
;
...
$GDXIN
$call CSV2GDX %indir%\%NG_price_instance%.csv Output=%indir%\%NG_price_instance%.gdx ID=NG_price UseHeader=y Index=1 Values=(2..LastCol)
parameter NG_price(interval,devices)   "Natural gas price in each interval ($/MMBtu)"
$GDXIN %indir%\%NG_price_instance%.gdx
$LOAD NG_price
$GDXIN
$call CSV2GDX %indir%\%load_prof_instance%.csv Output=%indir%\%load_prof_instance%.gdx ID=Load_profile UseHeader=y Index=1 Values=(2..LastCol)
parameter Load_profile(interval,devices)   "additional load profiles (MW)"
...
parameter renewable_signal(interval,devices)   "normalized renewable production profiles (MW)"
...
Renewable_power(interval,devices) = renewable_signal(interval,devices) * Renewable_MW(devices);
...
loop(devices,
         ...
         if ( renew_lifetime(devices)=0,  renew_lifetime(devices)=1;  renew_cap_cost(devices)=0;  renew_FOM_cost(devices)=0;  renew_VOM_cost(devices)=0; renew_interest_rate(devices)=0.01;);
...
Positive Variables
         ...
         output_power_MW_ren(interval,devices)   actual amount of renewable generation sold (MWh)
...
Equations
         ...
         input_ren_contribution(interval,devices)        equation that sets the amount of renewable gen consumed
...
************
         ...
         Fixed_dem_Apr(Month_Apr,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_May(Month_May,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Jun(Month_Jun,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Jul(Month_Jul,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Aug(Month_Aug,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Sep(Month_Sep,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Oct(Month_Oct,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Nov(Month_Nov,devices)     equation that enforces fixed demand charge in given month
         Fixed_dem_Dec(Month_Dec,devices)     equation that enforces fixed demand charge in given month

         Jan_1_eqn(Jan_1,devices)       equation that enforces timed demand charge in given month
         Feb_1_eqn(Feb_1,devices)       equation that enforces timed demand charge in given month
         Mar_1_eqn(Mar_1,devices)       equation that enforces timed demand charge in given month
         Apr_1_eqn(Apr_1,devices)       equation that enforces timed demand charge in given month
         May_1_eqn(May_1,devices)       equation that enforces timed demand charge in given month
         Jun_1_eqn(Jun_1,devices)       equation that enforces timed demand charge in given month
         Jul_1_eqn(Jul_1,devices)       equation that enforces timed demand charge in given month
         Aug_1_eqn(Aug_1,devices)       equation that enforces timed demand charge in given month
         Sep_1_eqn(Sep_1,devices)       equation that enforces timed demand charge in given month
         Oct_1_eqn(Oct_1,devices)       equation that enforces timed demand charge in given month
         Nov_1_eqn(Nov_1,devices)       equation that enforces timed demand charge in given month
         Dec_1_eqn(Dec_1,devices)       equation that enforces timed demand charge in given month

         Jan_2_eqn(Jan_2,devices)       equation that enforces timed demand charge in given month
         Feb_2_eqn(Feb_2,devices)       equation that enforces timed demand charge in given month
         Mar_2_eqn(Mar_2,devices)       equation that enforces timed demand charge in given month
         Apr_2_eqn(Apr_2,devices)       equation that enforces timed demand charge in given month
         May_2_eqn(May_2,devices)       equation that enforces timed demand charge in given month
         Jun_2_eqn(Jun_2,devices)       equation that enforces timed demand charge in given month
         Jul_2_eqn(Jul_2,devices)       equation that enforces timed demand charge in given month
         Aug_2_eqn(Aug_2,devices)       equation that enforces timed demand charge in given month
         Sep_2_eqn(Sep_2,devices)       equation that enforces timed demand charge in given month
         Oct_2_eqn(Oct_2,devices)       equation that enforces timed demand charge in given month
         Nov_2_eqn(Nov_2,devices)       equation that enforces timed demand charge in given month
         Dec_2_eqn(Dec_2,devices)       equation that enforces timed demand charge in given month

         Jan_3_eqn(Jan_3,devices)       equation that enforces timed demand charge in given month
         Feb_3_eqn(Feb_3,devices)       equation that enforces timed demand charge in given month
         Mar_3_eqn(Mar_3,devices)       equation that enforces timed demand charge in given month
         Apr_3_eqn(Apr_3,devices)       equation that enforces timed demand charge in given month
         May_3_eqn(May_3,devices)       equation that enforces timed demand charge in given month
         Jun_3_eqn(Jun_3,devices)       equation that enforces timed demand charge in given month
         Jul_3_eqn(Jul_3,devices)       equation that enforces timed demand charge in given month
         Aug_3_eqn(Aug_3,devices)       equation that enforces timed demand charge in given month
         Sep_3_eqn(Sep_3,devices)       equation that enforces timed demand charge in given month
         Oct_3_eqn(Oct_3,devices)       equation that enforces timed demand charge in given month
         Nov_3_eqn(Nov_3,devices)       equation that enforces timed demand charge in given month
         Dec_3_eqn(Dec_3,devices)       equation that enforces timed demand charge in given month

         Jan_4_eqn(Jan_4,devices)       equation that enforces timed demand charge in given month
         Feb_4_eqn(Feb_4,devices)       equation that enforces timed demand charge in given month
         Mar_4_eqn(Mar_4,devices)       equation that enforces timed demand charge in given month
         Apr_4_eqn(Apr_4,devices)       equation that enforces timed demand charge in given month
         May_4_eqn(May_4,devices)       equation that enforces timed demand charge in given month
         Jun_4_eqn(Jun_4,devices)       equation that enforces timed demand charge in given month
         Jul_4_eqn(Jul_4,devices)       equation that enforces timed demand charge in given month
         Aug_4_eqn(Aug_4,devices)       equation that enforces timed demand charge in given month
         Sep_4_eqn(Sep_4,devices)       equation that enforces timed demand charge in given month
         Oct_4_eqn(Oct_4,devices)       equation that enforces timed demand charge in given month
         Nov_4_eqn(Nov_4,devices)       equation that enforces timed demand charge in given month
         Dec_4_eqn(Dec_4,devices)       equation that enforces timed demand charge in given month

         Jan_5_eqn(Jan_5,devices)       equation that enforces timed demand charge in given month
         Feb_5_eqn(Feb_5,devices)       equation that enforces timed demand charge in given month
         Mar_5_eqn(Mar_5,devices)       equation that enforces timed demand charge in given month
         Apr_5_eqn(Apr_5,devices)       equation that enforces timed demand charge in given month
         May_5_eqn(May_5,devices)       equation that enforces timed demand charge in given month
         Jun_5_eqn(Jun_5,devices)       equation that enforces timed demand charge in given month
         Jul_5_eqn(Jul_5,devices)       equation that enforces timed demand charge in given month
         Aug_5_eqn(Aug_5,devices)       equation that enforces timed demand charge in given month
         Sep_5_eqn(Sep_5,devices)       equation that enforces timed demand charge in given month
         Oct_5_eqn(Oct_5,devices)       equation that enforces timed demand charge in given month
         Nov_5_eqn(Nov_5,devices)       equation that enforces timed demand charge in given month
         Dec_5_eqn(Dec_5,devices)       equation that enforces timed demand charge in given month

         Jan_6_eqn(Jan_6,devices)       equation that enforces timed demand charge in given month
         Feb_6_eqn(Feb_6,devices)       equation that enforces timed demand charge in given month
         Mar_6_eqn(Mar_6,devices)       equation that enforces timed demand charge in given month
         Apr_6_eqn(Apr_6,devices)       equation that enforces timed demand charge in given month
         May_6_eqn(May_6,devices)       equation that enforces timed demand charge in given month
         Jun_6_eqn(Jun_6,devices)       equation that enforces timed demand charge in given month
         Jul_6_eqn(Jul_6,devices)       equation that enforces timed demand charge in given month
         Aug_6_eqn(Aug_6,devices)       equation that enforces timed demand charge in given month
         Sep_6_eqn(Sep_6,devices)       equation that enforces timed demand charge in given month
         Oct_6_eqn(Oct_6,devices)       equation that enforces timed demand charge in given month
         Nov_6_eqn(Nov_6,devices)       equation that enforces timed demand charge in given month
         Dec_6_eqn(Dec_6,devices)       equation that enforces timed demand charge in given month
         ...
         system_power_eqn(interval,devices)      "sets the maximum system power level (e.g., inverter size for solar+storage)"
...
NG_price(interval,devices) = NG_price(interval,devices) * NG_price_adj(devices);
...
operating_profit_eqn..
         operating_profit =e= sum( (interval)$( rolling_window_min_index <= ord(interval) and ord(interval) <= rolling_window_max_index ),
                   (elec_sale_price_forecast(interval) * (sum(devices,output_power_MW(interval,devices)+output_power_MW_ren(interval,devices))) * interval_length)
                 + REC_price * sum(devices,output_power_MW_ren(interval,devices)) * interval_length
                 ...
                 - sum(devices,output_heat_rate(devices) * NG_price(interval,devices) * output_power_MW(interval,devices)) * interval_length
                 - sum(devices,input_heat_rate(devices) * NG_price(interval,devices) * input_power_MW(interval,devices)) * interval_length
                 ...
                 + sum(devices,Grid_CarbInt*LCFS_price*H2_EneDens*(power(10,-6)) * input_power_MW_ren(interval,devices) * interval_length / H2_LHV)
                 - sum(devices,renew_VOM_cost(devices) * Renewable_power(interval,devices))* interval_length
...
system_power_eqn(interval,devices)$( rolling_window_min_index <= ord(interval) and ord(interval) <= rolling_window_max_index )..
         output_power_MW(interval,devices) + output_power_MW_ren(interval,devices) =l= max_sys_output_cap;
...
input_capacity_limit_eqn2(interval,devices)$( rolling_window_min_index <= ord(interval) and ord(interval) <= rolling_window_max_index )..
         input_power_MW_ren(interval,devices)-Load_profile(interval,devices)+input_power_MW_non_ren(interval,devices) =e= input_power_MW(interval,devices);
...
input_ren_contribution(interval,devices)$( rolling_window_min_index <= ord(interval) and ord(interval) <= rolling_window_max_index )..
         input_power_MW_ren(interval,devices) + output_power_MW_ren(interval,devices) =l= Renewable_power(interval,devices);
...
if( (arbitrage_and_AS.modelstat=1 or arbitrage_and_AS.modelstat=2 or arbitrage_and_AS.modelstat=8),
         ...
         put input_echo_file;
                 ...
                 put /;
                 put 'Int, Elec Purchase ($/MWh), Elec Sale ($/MWh), Reg Up ($/MW), Reg Dn ($/MW), Spin Res ($/MW), Nospin Res ($/MW), Nat Gas ($/MMBTU), H2 ($/kg), Renewable In (MW), Load Profile (MW), Input Cap, Output Cap, Meter ($/mth)' /;
*                put 'Int, Elec Purchase ($/MWh), Elec Sale ($/MWh), Reg Up ($/MW), Reg Dn ($/MW), Spin Res ($/MW), Nospin Res ($/MW), Nat Gas ($/MMBTU), H2 ($/kg), Renewable In (MW), Load Profile (MW), Meter ($/mth)' /;
                 loop(interval, put      ord(interval),',',
                                         ...
                                         Max_input_cap(interval),',',
                                         Max_output_cap(interval),',',
